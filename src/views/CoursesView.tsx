      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="text-center mb-16">
          <motion.div 
            ...
            className="inline-flex items-center gap-2 bg-emerald-100 text-emerald-700 px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-[0.2em] mb-6 border border-emerald-200"
          >
            <Target size={14} /> Career Path Guidance
          </motion.div>
          <h1 className="text-4xl md:text-5xl font-black text-slate-900 mb-6 tracking-tight uppercase">DMT &amp; B.Sc <span className="text-emerald-600">Admission Coaching</span></h1>
          <p className="text-slate-500 italic max-w-3xl mx-auto leading-relaxed text-sm md:text-base">
            We provide official descriptions of these departments to help you choose your career path after passing the IHT admission. 
            <span className="block mt-2 font-black text-slate-900 not-italic">Note: We provide admission coaching only, not DMT academic courses.</span>
          </p>
        </div>
              ...
              <div className="relative h-64 overflow-hidden">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-slate-900/80 via-transparent to-transparent flex items-end p-8">
                   <div className="text-white">
                      ...
                      <h2 className="text-lg font-black leading-tight uppercase tracking-tight">
              ...
              <div className="p-8 flex flex-col flex-1">
                <div className="flex flex-wrap gap-2 mb-6">
                  <div className="bg-slate-50 text-slate-500 px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider flex items-center gap-1.5 border border-slate-100">
                    <Clock size={12} /> {i18n.language === 'bn' ? (course.durationBn || course.duration) : course.duration}
                  </div>
                ...
                <div className="mt-auto space-y-6">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 pt-6 border-t border-slate-50">
                     ...
                     >
                       {i18n.language === 'bn' ? 'বিস্তারিত দেখুন' : 'Apply Now'} <ArrowRight size={14} />
                     </button>
                     <button 
                     ...
                     >
                       {expandedId === course.id 
                         ? (i18n.language === 'bn' ? 'বন্ধ করুন' : 'Hide Specs') 
                         : (i18n.language === 'bn' ? 'বিস্তারিত তথ্য' : 'View Specs')}
                    ...
                    <div className="p-8 space-y-6">
                       ...
                       <div className="pt-6 border-t border-slate-200">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {course.detailedCurriculum?.map((year, idx) => (
                              <div key={idx} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm transition-all hover:border-blue-200 group/cur">
                                <div className="text-[9px] font-black text-blue-600 uppercase tracking-widest mb-3 flex justify-between items-center">
                                  <span>{i18n.language === 'bn' ? (year.yearBn || year.year) : year.year}</span>
                                  <div className="w-1.5 h-1.5 rounded-full bg-blue-100 group-hover/cur:bg-blue-500 transition-colors"></div>
                                </div>
                                <ul className="space-y-2">
                                  {year.subjects.map((sub, sIdx) => (
                                    <li key={sIdx} className="text-[10px] font-bold text-slate-600 flex items-center gap-2 leading-tight">
                                      <div className="w-1 h-1 rounded-full bg-slate-200"></div>
                                      {i18n.language === 'bn' && year.subjectsBn && year.subjectsBn[sIdx] ? year.subjectsBn[sIdx] : sub}
                                    </li>
                                  ))}
                                </ul>
                              </div>
